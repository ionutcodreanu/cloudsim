...
public class CloudSimExample9 {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Datacenter datacenter0 = createDatacenter("Datacenter_0", 5);
      @SuppressWarnings("unused")
...
//			FailureGenerator failure = new FailureGenerator("failure_injector", distr);
  ...
  private static void printHosts(List<Host> hostList) {
    int size = hostList.size();
    Host host;

    String indent = "    ";
    Log.printLine();
    Log.printLine("========== OUTPUT HOST LIST ==========");
    Log.printLine("Host ID" + indent + "STATUS" + indent +
        "Data center ID" + indent + "No of Vm" + indent + indent);

    for (int i = 0; i < size; i++) {
      String status = "SUCCESS";
      host = hostList.get(i);
      Log.print(host.getId() + indent + indent);
      if (host.isFailed()) {
        status = "FAILED ";
      }
      Log.print(status + indent);
      Log.printConcatLine(indent, host.getDatacenter().getId(), indent + indent + indent + indent,
          host.getVmList().size(),
          indent + indent);
    }
  ...
  private static Datacenter createDatacenter(String name, int noOfHosts) {
    ...
    try {
      datacenter = new DatacenterWithFailure(name, characteristics,
